...
function tryPlayJW(ch, list, idx){
  if (idx >= list.length) {
    ...
    if (window.__setNowPlaying) window.__setNowPlaying(`เล่นไม่สำเร็จ: ${ch?.name||''}`);
...
function enableAutoSwapHeaderOnScroll(){
  let lastY = 0;
  const h = document.querySelector('header');
  if (!h) return;
  const onScroll = ()=> {
    const y = window.scrollY || 0;
    const up = y < lastY; lastY = y;
    h.classList.toggle('show-shadow', y>0);
    h.classList.toggle('scrolling-up', up);
  };
  onScroll();
...
function getIconSVG(n){
  ...
  switch(n){
    case 'IPTV':
      return `<svg viewBox="0 0 24 24" fill="${c}"><path d="M4 2...-4h12V8H6v4zm4-9 2 2 2-2 1.4 1.4L12 6.8 8.6 4.4 10 3z"/></svg>`;
    case 'เด็ก':
      return `<svg viewBox="0 0 24 24" fill="${c}"><path d="M12 ...5-7a5 5 0 0 1 5-5zm1 14c2 3 1 4-1 6h-1l1-3-2 1 2-4h1z"/></svg>`;
    case 'บันเทิง':
      return `<svg viewBox="0 0 24 24" fill="${c}"><path d="M12 ...6 1.1 6.6L12 18.7 6.2 21l1.1-6.6-4.7-4.6 6.5-.9L12 3z"/></svg>`;
    case 'กีฬา':
      return `<svg viewBox="0 0 24 24" fill="${c}"><path d="M12 ...a8 8 0 0 1 6.9 12.1A8 8 0 0 1 5.1 7.1 8 8 0 0 1 12 4z"/></svg>`;
    case 'สารคดี':
      return `<svg viewBox="0 0 24 24" fill="${c}"><path d="M4 5... 0-3 3V5zm10-3h3a3 3 0 0 1 3 3v18a3 3 0 0 0-3-3h-3V2z"/></svg>`;
    case 'หนัง':
      return `<svg viewBox="0 0 24 24" fill="${c}"><path d="M21 ....2L16.8 4.1H19a2 2 0 0 1 2 2v2H3V6.1a2 2 0 0 1 1.7-2z"/></svg>`;
    default:
...
function render(opt={withEnter:false}){
  ...
  if (list.length===0){
    showEmptyState('ไม่มีช่องในหมวดนี้');
    return;
  }
...
function computeGridCols(container){
  ...
  return Math.max(1, Math.floor((inner + gap) / (tileW + gap)));
}
...
/* ------------------------ Histats (ย้ายเข้า holder ด้วย MutationObserver) ------------------------ */
function mountHistatsTopRight(){
  const holder = document.getElementById('histats_counter');
  if (!holder) return;
  const tryMove = ()=>{
    const c = document.getElementById('histatsC');
    if (c && !holder.contains(c)) holder.appendChild(c);
  };
  tryMove();
...
async function clearAppCache(){
  // ล้างเฉพาะคีย์ jwplayer*, HLS*, DASH* (รักษา lastId)
  try{
    const toDelete = [];
    for (let i=0;i<localStorage.length;i++){
      const k = localStorage.key(i) || '';
      if (/^(jwplayer|jwSettings|hls|dash)/i.test(k)) toDelete.push(k);
    }
    toDelete.forEach(k=> localStorage.removeItem(k));
  }catch{}
  // ล้าง cache storage (ถ้ามี service worker)
  if ('caches' in window) {
    try{
...
function scheduleAutoClear(){
  ...
  const AUTO_CLEAR_KEY = 'autoClearAt';
  const last = Number(safeGet(AUTO_CLEAR_KEY)) || 0;
  const now  = Date.now();
