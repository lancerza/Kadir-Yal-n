/* ========================= app.js (FINAL – refresh + auto-clear, no local cache) =========================
   - อ่าน categories.json + channels.json (กันแคชฝั่งเบราว์เซอร์ด้วย _t=timestamp)
   ...
   - NEW: ปุ่ม “รีเฟรช + ล้างแคช” + ล้างแคชอัตโนมัติทุก 6 ชม. (ไม่พึ่งเซิร์ฟเวอร์)
   - REMOVED: localStorage cache TV_DATA_CACHE_V1
=========================================================================================================== */
...
// ----- NEW: Auto clear every 6 hours (client-side only) -----
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  mountRefreshButton();        // NEW: ปุ่มรีเฟรช + ล้างแคช
  scheduleAutoClear();         // NEW: ล้างแคชอัตโนมัติทุก 6 ชม.

  await loadData();            // โหลดสดทุกครั้ง (ไม่มีแคช TV_DATA_CACHE_V1 แล้ว)
...
/* ------------------------ Load (no local cache) ------------------------ */
// NEW: fetch JSON แบบกันแคช และบังคับ fresh
...
function mountRefreshButton(){
  ...
  // ติดปุ่มเข้าไปในมุมซ้ายบนของเฮดเดอร์ (Histats อยู่มุมขวาบน)
  wrap.appendChild(btn);
}

// ลบแคช/คีย์ที่เกี่ยวกับแอป + ลบ Cache Storage (ถ้ามี)
async function clearAppCache(opt={}){
  try {
    // คีย์ของแอป
    localStorage.removeItem('lastId');
    localStorage.removeItem('TV_DATA_CACHE_V1'); // legacy key (ล้างทิ้ง)
    // คีย์ของ JW Player ที่อาจเก็บไว้
    const keys = Object.keys(localStorage);
    for (const k of keys) if (/^jwplayer\./i.test(k) || k.includes('jwplayer')) localStorage.removeItem(k);
  } catch {}

  // ลบ cache storage (ถ้าไม่มี service worker ก็ไม่มีอะไรให้ลบ)
  if (window.caches) {
    try {
...
// ตั้งตารางล้างแคชอัตโนมัติทุก 6 ชม. แบบ client-side
function scheduleAutoClear(){
  const now = Date.now();
  const last = Number(safeGet(AUTO_CLEAR_KEY) || 0);

  // ถ้าห่าง ≥ 6 ชม. ให้ล้างทันทีตอนเปิดหน้า
  if (!last || (now - last) >= SIX_HR_MS) {
    clearAppCache({ silent:true });
    safeSet(AUTO_CLEAR_KEY, String(now));
  }

  // ตั้งเวลาให้ครบ 6 ชม.ถัดไป แล้วค่อยเริ่ม interval ทุก 6 ชม.
  const delay = Math.max(1000, SIX_HR_MS - ((now - last) % SIX_HR_MS || 0));
  setTimeout(function tick(){
    clearAppCache({ silent:true });
    safeSet(AUTO_CLEAR_KEY, String(Date.now()));
